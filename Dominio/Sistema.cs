...
namespace Dominio
{
    public class Sistema
    {
        ...
        public List<TipoGasto> Gastos => _gastos;
        public List<Pago> Pagos => _pagos;
        ...
        public void AgregarEquipo(Equipo equipo) => _equipos.Add(equipo);
        public void AgregarTipoGasto(TipoGasto tipoGasto) => _gastos.Add(tipoGasto);
        ...
        public void EliminarTipoGasto(TipoGasto tipo) => _gastos.Remove(tipo);
        #endregion
        ...
        public Usuario CrearUsuario(string unNombre, string unApellido, string unaContrasenia,
                                    Equipo unEquipoSeleccionado, DateTime unaFechaIncorporacion, string Rol)
        {
            Usuario usuario = CrearUsuarioPorRol(Rol, unNombre, unApellido, unaContrasenia,
                                                 unEquipoSeleccionado, unaFechaIncorporacion);

            GenerarEmailValido(usuario);

            try { AgregarUsuario(usuario); }
            catch { }
                                           ...
                                           Equipo equipo, DateTime fechaIng)
        {
            ...
            throw new ArgumentException($"Rol no válido: '{rol}'.");
        }
        #endregion
        ...
        public void GenerarEmailValido(Usuario unUsuario)
        {
            ...
            string parteNombre, parteApellido;

            if (unUsuario.Nombre.Length <= 3) parteNombre = unUsuario.Nombre;
            else parteNombre = unUsuario.Nombre.Substring(0, 3);

            if (unUsuario.Apellido.Length <= 3) parteApellido = unUsuario.Apellido;
            else parteApellido = unUsuario.Apellido.Substring(0, 3);

            string mailBase = (parteNombre + parteApellido).ToLower();
            string mail = mailBase + dominio;

            int contador = 1;
            while (_usuarios.Exists(u => u.Email == mail))
            {
                mail = mailBase + contador + dominio;
                contador++;
            }
        ...
        public List<Pago> ObtenerPagosDelMesPorEmail(string email)
        {
            ...
            foreach (Pago pago in _pagos)
            {
                    ...
                    pago.UsuarioQueRealizo.Email.Equals(email, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    {
                        if (unPago.FechaPago.Month == mesActual && unPago.FechaPago.Year == anioActual)
                            pagosDelMes.Add(pago);
                    }
                    else if (pago is Recurrente rec)
                    {
                        if (rec.FechaInicio.Month == mesActual && rec.FechaInicio.Year == anioActual)
                            pagosDelMes.Add(pago);
                    }
                }
            }
        ...
        #region Pagos por equipo por mes
        public List<Pago> ObtenerPagosEquipoPorMes(Usuario gerente, int mes, int anio)
        {
            ...
            foreach (Pago pago in _pagos)
            {
                ...
                {
                    bool activoEnMes =
                        r.FechaInicio <= finMes &&
                        (r.FechaFin == DateTime.MaxValue || r.FechaFin >= inicioMes);

                    if (activoEnMes) resultado.Add(pago);
                }
            }
            ...
            return resultado;
                ...
                // Calcula el total base (sin beneficios) de una lista de pagos
        ...
        // Calcula el total con beneficios (descuentos / recargos de Unico y Recurrente)
